...
from utils import create_training_data, calculate_ROC, NUM_OF_VERTICES
...
def train_model(model_semnet, data_train0, data_train1, data_test0, data_test1, lr_enc, batch_size):
    ...
    for iteration in range(50000): # should be much larger, with good early stopping criteria
        ...
        with torch.no_grad():
            ...
            if iteration%50==0:
                info_str='iteration: '+str(iteration)+' - train loss: ',np.mean(all_real_loss[0])+np.mean(all_real_loss[1]),'; test loss: ',np.mean(all_real_loss[2])+np.mean(all_real_loss[3])
...
def link_prediction_semnet(full_dynamic_graph_sparse,unconnected_vertex_pairs,year_start,years_delta):
    """
    ...
    :param years_delta, Integer, number oif years to predict,
    ...
    edges_used=5*10**7 # Best would be to use all vertices, to create more training data. But that takes long and requires huge amount of memory. So here we use a random subset.
    ...
    batch_size=250 # Large batch_size seems to be important
    lr_enc=2.5*10**-4
